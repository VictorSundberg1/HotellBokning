...
exports.handler = async (event) => {
	try {
		const { name, epost, guestCount, bookedRooms, checkInDate, checkOutDate } =
		...
		for (const room of bookedRooms) {
			...
			const roomData = roomsResult.Items.find((r) => r.id === room);
		...
		if (totalCapacity < guestCount) {
			return sendResponse(400, {
				message: `Valda rum har endast plats för ${totalCapacity} gäst(er), Välj större eller fler rum för ${guestCount} gäster.`,
			});
		}

		const existing = await db.send(
			new ScanCommand({ TableName: 'booking-table' })
		);
